...
class Datasets:
    ...
    def first_pruning(self):
        ...
        for symbol in self.all_symbols:
            ...
            wacc = Wacc(self.bestk.P, self.bestk.N, len(sup_pos), len(sup_neg))
            min_wacc = sys.maxsize
            in_list = False
            if len(best_unique) < self.bestk.k:
                best_unique.append(wacc)
            else:
                for s in best_unique:
                    if s == wacc:
                        in_list = True
                    min_wacc = min(min_wacc, s)
                if not in_list and min_wacc < wacc:
                    best_unique.remove(min_wacc)
                    best_unique.append(wacc)

        new_pos = dict()
        new_neg = dict()

        min_wacc = min(best_unique)
        ...
        for symbol in self.all_symbols:
            sup_pos = self.pos.vertical_first.get((symbol,)[0], [])
            sup_neg = self.neg.vertical_first.get((symbol,)[0], [])
            threshold = min_wacc * self.bestk.P
            threshold2 = min_wacc * self.bestk.N
            if Wacc(self.bestk.P, self.bestk.N, len(sup_pos), len(sup_neg)) >= min_wacc or len(sup_pos) >= threshold or len(sup_neg) >= threshold2:
...
class BestK:
    ...
    def add_frequent(self, sequence, support_pos, support_neg):
        wacc = Wacc(self.P, self.N, support_pos, support_neg)
...
# python3 supervised_closed_sequence_mining_absolute_wacc.py Datasets/Protein/PKA_group15.txt Datasets/Protein/SRC1521.txt 6
# python3 supervised_closed_sequence_mining_absolute_wacc.py Datasets/Test/positive.txt Datasets/Test/negative.txt 6
# python3 supervised_closed_sequence_mining_absolute_wacc.py Datasets/Reuters/earn.txt Datasets/Reuters/acq.txt 1
