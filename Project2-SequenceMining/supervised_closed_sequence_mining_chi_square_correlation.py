...
class Datasets:
    ...
    def first_pruning(self):
        ...
        for symbol in self.all_symbols:
            ...
            chi = Chi_square(self.bestk.P, self.bestk.N, len(sup_pos), len(sup_neg))
            min_chi = sys.maxsize
            in_list = False
            if len(best_unique) < self.bestk.k:
                best_unique.append(chi)
            else:
                for s in best_unique:
                    if s == chi:
                        in_list = True
                    min_chi = min(min_chi, s)
                if not in_list and min_chi < chi:
                    best_unique.remove(min_chi)
                    best_unique.append(chi)

        new_pos = dict()
        new_neg = dict()

        min_chi = min(best_unique)
        ...
        for symbol in self.all_symbols:
            sup_pos = self.pos.vertical_first.get((symbol,)[0], [])
            sup_neg = self.neg.vertical_first.get((symbol,)[0], [])
            if Chi_square(self.bestk.P, self.bestk.N, len(sup_pos), len(sup_neg)) >= min_chi or Chi_square(self.bestk.P, self.bestk.N, 0, len(sup_neg)) >= min_chi or Chi_square(self.bestk.P, self.bestk.N, len(sup_pos), 0) >= min_chi:
...
class BestK:
    ...
    def add_frequent(self, sequence, support_pos, support_neg):
        chi = Chi_square(self.P, self.N, support_pos, support_neg)
        if chi < self.min_Chi:
            return
        if len(self.best_k) < self.k:
            ...
            for i in range(len(self.best_k)):
                ...
                if support == chi:
                    ...
                    return
            # Not in there
            self.best_k.append(([(sequence, support_pos, support_neg)], chi))
...
def Chi_square(P, N, p, n):
    E = [P*(p+n)/(P+N), P*(P+N-p-n)/(P+N), N*(p+n)/(P+N), N*(P+N-p-n)/(P+N)]
    O = [p, P-p, n, N-n]
    chi = 0
...
def dfs(sequence, bestk, dss, proj_pos, proj_neg):
    ...
    for symbol in dss.all_symbols:
        ...
        if a[0] in proj_pos.keys() or a[0] in proj_neg.keys():
            ...
            # Threshold set to 0
            chi = Chi_square(bestk.P, bestk.N, support_pos, support_neg)
            if chi >= bestk.min_Chi or Chi_square(bestk.P, bestk.N, 0, support_neg) >= bestk.min_Chi or Chi_square(bestk.P, bestk.N, support_pos, 0) >= bestk.min_Chi:
...
# python3 supervised_closed_sequence_mining_chi_square_correlation.py Datasets/Protein/PKA_group15.txt Datasets/Protein/SRC1521.txt 6
# python3 supervised_closed_sequence_mining_chi_square_correlation.py Datasets/Test/positive.txt Datasets/Test/negative.txt 6
# python3 supervised_closed_sequence_mining_chi_square_correlation.py Datasets/Reuters/earn.txt Datasets/Reuters/acq.txt 1
